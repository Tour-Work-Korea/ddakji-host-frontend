...
/**
 * LocalTime (hour:minute:second) → "HH:mm"
 * null, undefined 또는 올바르지 않은 시간은 빈 문자열을 반환합니다.
 */
export const formatLocalTimeToHourMinute = localTime => {
  if (!localTime) return '';

  const match = String(localTime).match(/^(\d{1,2}):(\d{2})/);
  if (!match) return '';
